...
If the retrieval mode is "similarity" and a finding was surfaced, note the similarity score and the matched hash in your findings. A score below 1.0 means this is a partial match — the finding may describe a different variant of the same or a related incident.

### Outcome history

If a prior finding was loaded, check whether it carries outcome data:
- `outcome`: whether the prior fix was merged, modified, or rejected.
- `verdict`: what the merge revealed about the prior diagnosis.
- `lesson`: a plain-language takeaway from the merge outcome.

If outcome data is present, include it in your Investigation section:
- `> **Prior outcome:** ${outcome} — ${verdict}`
- `> **Learned lesson:** ${lesson}`

Use the lesson as additional context alongside the finding — it represents grounded feedback from a real merge, not just the agent's own prior opinion. But it is still subject to the same validation discipline as the finding itself.
...
**Corroboration rule:** Even when VALIDATED, you must cross-reference at least two independent signal types (logs + traces, logs + code, traces + code, or all three) before stating a root cause. A single signal is not sufficient. Read upward from the throw site to the cause — follow the code path that leads to the exception, not just the exception itself.
...
3. Correlate logs with traces to find the root cause. Read upward from the throw to the cause — follow the code path, not just the exception.
