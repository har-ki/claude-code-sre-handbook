"""
Storage interface for the complete SRE watcher (context + learning).

Outcome-bearing findings from the start: each finding carries outcome,
verdict, and lesson fields alongside the finding text and embedding.

Provides recall() and persist() across two backends:
  - JsonBackend: flat JSON vector index
...
VALID_OUTCOMES = {"pending", "merged", "merged_modified", "closed_unmerged"}
...
class StorageBackend(ABC):
    """Abstract storage interface for incident findings with outcome tracking."""

    @abstractmethod
    def recall(self, service: str, exception_class: str, fp_hash: str,
               mode: str = "exact", threshold: float = 0.75) -> dict:
        """Retrieve a prior finding and its outcome.
    ...
    def persist(self, fp_hash: str, service: str, exception_class: str,
                finding_text: str, outcome: str = "pending",
                verdict: str | None = None,
                lesson: str | None = None) -> bool:
        """Store a finding and update the embedding index. Returns success."""

    @abstractmethod
    def record_outcome(self, fp_hash: str, outcome: str,
                       verdict: str, lesson: str) -> bool:
        """Update an existing finding's outcome fields in place.
...
class JsonBackend(StorageBackend):
    """Flat JSON vector index with outcome fields."""
    ...
    def _get_outcome_fields(self, fp_hash: str) -> dict:
        ...
        return {"outcome": None, "verdict": None, "lesson": None}
    ...
    def recall(self, service: str, exception_class: str, fp_hash: str,
        ...
        if mode == "exact":
            ...
            outcome_fields = self._get_outcome_fields(fp_hash) if content else {
                "outcome": None, "verdict": None, "lesson": None
            }
            return {
                "content": content,
                "score": score,
            ...
            outcome_fields = self._get_outcome_fields(matched_hash) if content else {
                "outcome": None, "verdict": None, "lesson": None
            }
            return {
                "content": content,
...
class SqliteBackend(StorageBackend):
    """SQLite-backed storage with atomic writes and outcome tracking.

    Schema: findings(fp_hash PK, service, exception_class, finding_text,
                     embedding BLOB, timestamp REAL,
                     outcome TEXT, verdict TEXT, lesson TEXT, outcome_ts REAL)

    Cosine search is done in Python after loading candidate rows — SQLite
    provides atomicity and consistency, not vector search.
    """
    ...
    def _get_outcome_fields(self, fp_hash: str) -> dict:
        with self._connect() as conn:
            row = conn.execute(
                "SELECT outcome, verdict, lesson FROM findings WHERE fp_hash = ?",
                (fp_hash,)
            ).fetchone()
        if row:
            return {"outcome": row[0], "verdict": row[1], "lesson": row[2]}
        return {"outcome": None, "verdict": None, "lesson": None}
    ...
    def persist(self, fp_hash: str, service: str, exception_class: str,
        ...
        with self._connect() as conn:
            ...
            """, (fp_hash, service, exception_class, finding_text,
                  blob, time.time(), outcome, verdict, lesson))
        return True
    ...
    def record_outcome(self, fp_hash: str, outcome: str,
        ...
        with self._connect() as conn:
            cursor = conn.execute("""
                UPDATE findings
                SET outcome = ?, verdict = ?, lesson = ?, outcome_ts = ?
                WHERE fp_hash = ?
            """, (outcome, verdict, lesson, time.time(), fp_hash))
