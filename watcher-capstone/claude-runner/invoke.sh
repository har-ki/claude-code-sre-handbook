...
# Capstone variant: /memory-store/ mounted with incidents/ and embeddings/.
#
# Required env: PHASE, FP, FP_HASH, FINGERPRINT, SERVICE, RATE_PCT,
#               PR_NUM, GITHUB_REPO, MODEL, DATE
# Phase 1 also uses: PRIOR_FINDING, RETRIEVAL_INFO (retrieval metadata)
...
# gh pr ready is deliberately absent from both phases.
if [[ "$PHASE" == "1" ]]; then
