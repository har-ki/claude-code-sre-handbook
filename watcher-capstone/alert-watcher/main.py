#!/usr/bin/env python3
"""
Alert watcher (capstone) — polls ClickHouse for elevated error rates,
...
Integrates the store.py storage interface (JsonBackend or SqliteBackend),
configurable retrieval modes (exact/similarity), validation discipline,
and human-gated write-back.

Consolidation vs watcher-semantic-example:
  - memory-store/  holds ONLY per-fingerprint learnings (<sha8>.md + embeddings)
...
def run_phase(phase: int, fp: str, fp_hash: str, service: str,
    ...
    runner_image = os.environ.get("RUNNER_IMAGE", "watcher-capstone-claude-runner")
...
def append_incident_record(fp: str, fp_hash: str, pr_url: str,
    ...
    Note: human-readable incident reports (docs/incidents/...) are also
    conceptually part of incident-store — Phase 2 writes those directly
    into the workspace clone. This function handles the structured ledger.
    """
...
def main():
    ...
    log("watcher started (capstone)", repo=repo, poll_interval=POLL_INTERVAL,
    ...
    while True:
        try:
            ...
            for alert in alerts:
                ...
                # Memory retrieval (via store.py backend)
