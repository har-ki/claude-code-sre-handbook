...
Example — error count last 5 min:
```bash
kubectl get --raw "/api/v1/namespaces/clickhouse/services/clickhouse:8123/proxy/?query=SELECT%20count()%20FROM%20otel_logs%20WHERE%20Timestamp%20%3E%3D%20now()%20-%20INTERVAL%205%20MINUTE%20AND%20SeverityText%20%3D%20%27ERROR%27"
```
...
- **Phase 1:** Read `/memory-store/incidents/<hash>.md` at the start of investigation. If it exists, use it as prior context.
