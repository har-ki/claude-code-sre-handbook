#!/usr/bin/env python3
"""
Alert watcher (memory-enabled) — polls ClickHouse for elevated error rates,
deduplicates against open GitHub PRs, opens a draft PR, then triggers
claude-runner for Phase 1 (investigate) and Phase 2 (propose fix).

This variant adds a file-based memory store: Phase 1 reads prior findings
from memory-store/incidents/<sha8>.md, and Phase 2 writes them back.
"""
...
def validate_memory_path(fp_hash: str) -> str | None:
    ...
    if not re.match(r'^[0-9a-f]{8}$', fp_hash):
        log("invalid fp_hash for memory path", fp_hash=fp_hash)
        return None
    ...
    if not resolved.startswith(os.path.realpath(MEMORY_INCIDENTS_DIR) + os.sep):
        log("memory path traversal rejected", fp_hash=fp_hash, resolved=resolved)
        return None
    return path


# -- Memory store: read (before Phase 1) ------------------------------------
def read_memory(fp_hash: str) -> str | None:
    """Read prior finding for this fingerprint, if it exists."""
    ...
    if os.path.isfile(path):
        ...
        log("memory hit", fp_hash=fp_hash, path=path, content_len=len(content))
        return content
    log("memory miss", fp_hash=fp_hash)
    return None


# -- Memory store: verify write (after Phase 2) ------------------------------
def verify_memory_write(fp_hash: str) -> bool:
    """Check whether Phase 2 actually wrote to the memory store."""
    path = validate_memory_path(fp_hash)
    if path is None:
        return False
    if os.path.isfile(path):
...
def run_phase(phase: int, fp: str, fp_hash: str, service: str,
              ...
              evidence: str = "",
              detection: str = "", trigger_stats: dict | None = None,
              prior_finding: str | None = None) -> tuple[int, float]:
    ...
    env = {
        **os.environ,
        ...
        # Memory: pass prior finding content for Phase 1 context
    ...
    runner_image = os.environ.get("RUNNER_IMAGE", "watcher-memory-example-claude-runner")
...
def append_incident_record(fp: str, fp_hash: str, pr_url: str,
    ...
    log("incident record saved", fp_hash=fp_hash,
        memory_hit=memory_hit, memory_write_verified=memory_write_verified)
...
def main():
    ...
    # Ensure memory-store/incidents directory exists
    os.makedirs(MEMORY_INCIDENTS_DIR, exist_ok=True)

    ensure_workspace_clone(repo)
    log("watcher started (memory-enabled)", repo=repo, poll_interval=POLL_INTERVAL,
        threshold=THRESHOLD, max_turns=MAX_TURNS, model=model,
        memory_dir=MEMORY_INCIDENTS_DIR)
    ...
    while True:
        try:
            ...
            for alert in alerts:
                ...
                # Memory: read prior finding before Phase 1
                prior_finding = read_memory(fp_hash)
                memory_hit = prior_finding is not None

                # Phase 1: investigate — pass trigger stats and prior finding
                ...
                # Memory: verify Phase 2 wrote to memory store
                ...
                # Record to memory store (incidents.jsonl)
