#!/usr/bin/env python3
"""
Alert watcher (semantic-memory variant) — polls ClickHouse for elevated error
rates, deduplicates against open GitHub PRs, opens a draft PR, then triggers
claude-runner for Phase 1 (investigate) and Phase 2 (propose fix).

This variant adds configurable retrieval modes (exact-hash or similarity),
validation discipline (on/off), and human-gated write-back.

Config knobs (in config.yaml or env):
  retrieval_mode: exact | similarity
  similarity_threshold: 0.0–1.0 (similarity mode only)
  validation_discipline: true | false
"""
...
# ── Memory retrieval (configurable mode) ────────────────────────────
def read_memory(service: str, exception_class: str, fp_hash: str,
                mode: str, threshold: float) -> dict:
    """Retrieve prior finding using configured mode. Returns retrieval result dict."""
    result = retrieve(service, exception_class, fp_hash,
...
# ── Index a finding after write-back ────────────────────────────────
def index_finding(fp_hash: str, service: str, exception_class: str) -> bool:
    """Read the finding file and add/update its embedding in the vector index."""
    path = validate_memory_path(fp_hash)
    if path is None or not os.path.isfile(path):
        return False
    with open(path) as f:
        content = f.read().strip()
    ok = add_to_index(fp_hash, service, exception_class, content)
    if ok:
...
def run_phase(phase: int, fp: str, fp_hash: str, service: str,
              ...
              retrieval_info: dict | None = None) -> tuple[int, float]:
    """Shell out to claude-runner/invoke.sh. Returns (exit_code, duration_sec)."""
    date_str = datetime.now(timezone.utc).strftime("%Y-%m-%d")

    # Build retrieval context string for the prompt
    retrieval_context = ""
    if retrieval_info:
    ...
    runner_image = os.environ.get("RUNNER_IMAGE", "watcher-semantic-example-claude-runner")
...
def main():
    ...
    retrieval_mode = cfg.get("retrieval_mode", "exact")
    similarity_threshold = cfg.get("similarity_threshold", 0.75)
    validation_discipline = cfg.get("validation_discipline", False)

    os.makedirs(MEMORY_INCIDENTS_DIR, exist_ok=True)

    ensure_workspace_clone(repo)
    log("watcher started (semantic-memory)", repo=repo, poll_interval=POLL_INTERVAL,
        ...
        try:
            ...
            for alert in alerts:
                ...
                retrieval_result = read_memory(
                    service, top_error_class, fp_hash,
                ...
                # Phase 2: propose fix (memory write deferred to after human gate)
                ...
                if memory_write_ok:
                    index_finding(fp_hash, service, top_error_class)
